...
export const api = {
  ...
  monitoring: () => request("/api/monitoring"),
  tickets: status => request(`/api/tickets${status ? `?status=${encodeURIComponent(status)}` : ""}`),
  generateTicket: difficulty => request("/api/tickets/generate", {method: "POST", body: {difficulty}}),
  generateTickets: (difficulty, count) => request("/api/tickets/generate-batch", {method: "POST", body: {difficulty, count}}),
  startTicket: (ticketId, technician) => request(`/api/tickets/${encodeURIComponent(ticketId)}/start`, {method: "POST", body: {technician}}),
  resolveTicket: (ticketId, technician, resolutionNotes) => request(`/api/tickets/${encodeURIComponent(ticketId)}/resolve`, {method: "POST", body: {technician, resolution_notes: resolutionNotes}}),
  verifyTicket: ticketId => request(`/api/tickets/${encodeURIComponent(ticketId)}/verify`, {method: "POST"}),
  ticketFileConnectivity: ticketId => request(`/api/tickets/${encodeURIComponent(ticketId)}/file-connectivity`, {method: "POST"}),
  ...
  deleteDirectoryUser: username => request(`/api/directory/users/${encodeURIComponent(username)}`, {method: "DELETE"}),
  ...
  fileServerFault: action => request(`/api/fileserver/faults/${encodeURIComponent(action)}`, {method: "POST", body: {}}),
  fileShareFault: (share, action) => request(`/api/fileserver/faults/${encodeURIComponent(action)}`, {method: "POST", body: {share}}),
  fileServerRemediation: action => request(`/api/fileserver/actions/${encodeURIComponent(action)}`, {method: "POST"}),
  fileShareRemediation: (share, action) => request(`/api/fileserver/shares/${encodeURIComponent(share)}/${encodeURIComponent(action)}`, {method: "POST"}),
